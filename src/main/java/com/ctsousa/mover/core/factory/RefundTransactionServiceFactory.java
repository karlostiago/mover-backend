...
@Component
public class RefundTransactionServiceFactory implements TransactionExecutor {
    ...
    public RefundTransactionServiceFactory(IncomeService incomeService, ExpenseService expenseService, InvestimentService investimentService, CorporateCapitalService corporateCapitalService, TransferService transferService) {
    ...
    public TransactionEntity execute(final TypeCategory type, final Transaction transaction) {
        ...
        return mapServicies.get(type).refund(transaction.getId());
    }
}
