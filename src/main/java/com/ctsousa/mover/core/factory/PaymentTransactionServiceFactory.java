...
@Component
public class PaymentTransactionServiceFactory implements TransactionExecutor {
    ...
    public PaymentTransactionServiceFactory(IncomeService incomeService, ExpenseService expenseService, InvestimentService investimentService, CorporateCapitalService corporateCapitalService, TransferService transferService) {
    ...
    public TransactionEntity execute(final TypeCategory type, final Transaction transaction) {
        ...
        return mapServicies.get(type).payment(transaction.getId(), transaction.getPaymentDate());
    }
}
