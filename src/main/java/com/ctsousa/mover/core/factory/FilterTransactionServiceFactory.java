...
@Component
public class FilterTransactionServiceFactory implements TransactionExecutor {
    ...
    public FilterTransactionServiceFactory(IncomeService incomeService, ExpenseService expenseService, InvestimentService investimentService, CorporateCapitalService corporateCapitalService, TransferService transferService) {
    ...
    public TransactionEntity execute(final TypeCategory type, final Transaction transaction) {
        ...
        return mapServicies.get(type).filterById(transaction.getId());
    }
}
